...
pub fn deserialize_config(path: &Path, warn_pruned: bool) -> Result<Value> {
    ...
    if (extension == "yaml" || extension == "yml") && !contents.trim().is_empty() {
        warn!("YAML config {path:?} is deprecated, please migrate to TOML using `tabor migrate`");
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("tabor.{suffix}");
    ...
    xdg::BaseDirectories::with_prefix("tabor")
        ...
        .or_else(|| {
            if let Ok(home) = env::var("HOME") {
                // Fallback path: $HOME/.config/tabor/tabor.toml.
                let fallback = PathBuf::from(&home).join(".config/tabor").join(&file_name);
                if fallback.exists() {
                    return Some(fallback);
                }
                // Fallback path: $HOME/.tabor.toml.
            ...
            let fallback = PathBuf::from("/etc/tabor").join(&file_name);
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("tabor.{suffix}");
    dirs::config_dir().map(|path| path.join("tabor").join(file_name)).filter(|new| new.exists())
}
