...
pub const LOG_TARGET_IPC_CONFIG: &str = "tabor_log_window_config";

/// Name for the environment variable containing the log file's path.
const TABOR_LOG_ENV: &str = "TABOR_LOG";

/// Logging target for config error messages.
pub const LOG_TARGET_CONFIG: &str = "tabor_config_derive";

/// Logging target for winit events.
pub const LOG_TARGET_WINIT: &str = "tabor_winit_event";
...
impl Logger {
    ...
    fn message_bar_log(&self, record: &log::Record<'_>, logfile_path: &str) {
        ...
        #[cfg(not(windows))]
        let env_var = format!("${TABOR_LOG_ENV}");
        #[cfg(windows)]
        let env_var = format!("%{}%", TABOR_LOG_ENV);
...
impl OnDemandLogFile {
    fn new() -> Self {
        ...
        unsafe { env::set_var(TABOR_LOG_ENV, path.as_os_str()) };
