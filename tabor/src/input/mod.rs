...
pub trait ActionContext<T: EventListener> {
    ...
    #[cfg(target_os = "macos")]
    fn web_handle_key_release(&mut self, _key: &KeyEvent, _text: &str) -> bool {
        false
    }
    #[cfg(target_os = "macos")]
    ...
    fn execute<A: ActionContext<T>>(&self, ctx: &mut A) {
        match self {
            ...
            Action::ToggleFullscreen => {
                ...
                ctx.display().pending_update.dirty = true;
            },
            Action::ToggleMaximized => ctx.window().toggle_maximized(),
            #[cfg(target_os = "macos")]
            Action::ToggleSimpleFullscreen => {
                ctx.window().toggle_simple_fullscreen();
                ctx.display().pending_update.dirty = true;
            },
            #[cfg(target_os = "macos")]
...
mod tests {
    ...
    use tabor_terminal::event::Event as TerminalEvent;
